  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-logo-dp via-logo-gray to-logo-gray pt-10">
      <div className="container mx-auto px-4 py-8 max-w-8xl">
        ...
        {error && <div className="text-red-500 text-center mb-8">Failed to load projects. Please try again later.</div>}
              ...
              >
                ...
                <CardContent className="flex-1 flex flex-col justify-between p-6">
                  <CardHeader className="p-0 mb-2">
                    <CardTitle className="text-xl font-bold text-logo-yellow mb-1 line-clamp-2">{project.title}</CardTitle>
                  </CardHeader>
                  <CardDescription className="text-white mb-3 line-clamp-3">{project.summary}</CardDescription>
                ...
                .map(tech => (
                  <span key={tech} className="bg-logo-yellow text-logo-dp px-3 py-1 rounded-full text-xs font-semibold">{tech}</span>
                ))}
                    </div>
                  )}
                </CardContent>
